...
contract TimedVault {
    ...
    modifier only(address authorizedAddress) {
     ...
     */
    function TimedVault(address _recovery, uint _withdrawDelay) {
        owner = msg.sender;
        recovery = _recovery;
     ...
     */
    function unvault(uint amount) only(owner) {
     ...
     */
    function recover(address target) only(recovery) {
     ...
     */
    function lock(uint duration) only(owner) {
